...
def create_action(entry, index):
    actions_dic = {
        0:f"click_and_type('Where from?', '{entry['from_city']}')",
        1:f"click_and_type('Where to?', '{entry['to_city']}')",
        2:f"click_and_type('Departure date?', '{entry['from_date']}'" +"TAB)",
        3:f"type('Return date?', '{entry['to_date']}')",
        4:f"click('Done')",
        5:f"click('Search')",
        6:f"click('Cheapest')",
        7:f"scroll(250)"
